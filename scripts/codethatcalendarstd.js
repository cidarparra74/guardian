...
function UA(){var t=this,nv=navigator,n=nv.userAgent.toLowerCase();
...
if(c){c.date.setFullYear(y);
c.date.setMonth(m);
}
location.reload();};
function CodeThatFind(d,f){if(d.getElementById&&Def(d.getElementById(f))){
return d.getElementById(f);
}else if(ua.ie4up){
return d.all[f];
}else{if(d.forms.item)
...
if(hours<0||hours>23){hours=hours%24;
hours+=(hours<0?24:0);
}
...
if(seconds<0||seconds>59){seconds=seconds%60;seconds+=(seconds<0?60:0);}
...
if(Def(e)){e.value=CodeThatDateFormat(f,d,m,y);
