...
function buscarcli(xci,xem){
	...
	if(emision == '--') emision = '';
	//alert(xem.options[xem.selectedIndex].text);
	parametro = "variable=locclisec2&ci="+xci.value+"&emi="+emision+"&random=" + Math.random()*99999;
...
/* ------- funciones para las partes ------- */
	function agregarFila(obj){
		...
		//	var rol = document.getElementById("selRol");
			...
			/* if(emi.options[emi.selectedIndex].text=='--'){
				alert("Seleccione el lugar de emisi\u00F3n del documento por favor.");
				emi.focus();
				return false;
			} */
			if(nombre.value==''){
			...
			if(procede.value==''){
				alert("Ingrese la nacionalidad de la persona por favor.");
				procede.focus();
				return false;
			}
			if(estCivil.value=='-'){
				alert("Seleccione el estado civil de la persona por favor.");
				estCivil.focus();
				return false;
			}
			//alert(estCivil.value);
			if(redaccion.value==''){
				//alert("Ingrese la redaccion.");
				//procede.focus();
				//return false;
				doredaccion();
				redaccion = document.getElementById("txtRedaccion");
			}
			if(selRol.value=='-'){
