...
var ModalPopups = {
    ...
    _styleAllLayers: function(id, parameters, allLayers) {
        ...
        if(!ModalPopupsSupport.isOlderIE()) {
			...
			var viewport = ModalPopupsSupport.getViewportDimensions();
			oBackground.innerHTML = "<div><iframe style='z-index:-1; position:absolute; top:0;left:0 display:none; display/**/:block; position:absolute; filter:mask(); width:" + viewport.width + "px; height:" + viewport.height + "px;' id='corr_bug_ie' src='../images/actions/spinner.gif'></iframe></div>";
		}
