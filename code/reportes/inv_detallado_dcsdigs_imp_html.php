...
$sql= "SELECT p.mis, p.emision, p.nombres, t.tipo_bien, o.nombre, m.id_estado, m.flujo, convert(varchar,c.creacion_carpeta,103) as fecha, count(*) docs, SUM(IIF( archivo is not null, 1, 0)) digs
FROM (propietarios p 
INNER JOIN carpetas c ON c.id_propietario=p.id_propietario 
INNER JOIN documentos_propietarios dp ON dp.id_carpeta=c.id_carpeta
...
$consulta 
GROUP BY p.mis, p.emision, p.nombres, t.tipo_bien, o.nombre, m.id_estado, m.flujo, arch_corr_prest, c.creacion_carpeta
ORDER BY right(mis,1), right(mis,2), mis ";
...
$smarty->display('reportes/inv_detallado_dcsdigs_imp_html.html');
die();
?>