...
//cargando para el overlib
	...
	//href
	$carpeta_entrar="../code/_main.php?action=informe_legal_aprob.php";
	$smarty->assign('carpeta_entrar',$carpeta_entrar);
	//form
	$carpeta_acc= "informe_legal_aprob";
	...
	$sql= "SELECT * FROM tipos_bien WHERE categoria = 0 ORDER BY id_tipo_bien ";
	$query = consulta($sql);
	$i=0;
	$tiposbien=array();
	while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
...
/**********************valores para la ventana*************************/
	...
	//------------------
	//revertir la autorizacion
	if(isset($_REQUEST['eliminar'])){
		$cat = 0;
		include("./informe_legal/eliminar_solicitud.php");
	}
	//aceptando
	if(isset($_REQUEST['eliminar_boton_x'])){
		$aprobando = 'vrb';
		include("./informe_legal/eliminando_solicitud.php");
	}
...
il.nrocaso, p.nombres cliente, tb.tipo_bien, il.noportunidad
...
//para los aprobados
	...
	$sql= "SELECT il.*, us.nombres, tb.tipo_bien, tb.bien, ca.id_informe_legal as catastro,
	...
	INNER JOIN tipos_bien tb ON tb.id_tipo_bien=il.id_tipo_bien 
	LEFT JOIN carpetas ca ON ca.id_informe_legal=il.id_informe_legal 
	...
	AND il.estado = 'apr' 
	AND ofi.id_almacen = $id_almacen $armar_consulta ORDER BY il.id_informe_legal DESC ";
//echo $sql;
$query = consulta($sql);
...
while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$smarty->display('informe_legal_aprob.html');
	die();
