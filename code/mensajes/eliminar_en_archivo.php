...
require_once('../lib/fechas.php');
	...
	$sql= "SELECT m.id_carpeta, m.corr_auto, m.auto_arch, m.arch_corr_prest, 
	m.arch_corr_plazo, m.arch_corr_conf, m.corr_arch_ret, m.corr_arch_ret_conf, id_us_archivo,
	...
	WHERE id_movimiento_carpeta='$id_movimiento' AND m.flujo='0' AND m.id_estado='7' AND m.id_us_corriente=u.id_usuario ";
	//echo "$sql";
	$result= consulta($sql);
	...
	$fecha_corr_arch_ret_conf= fechaDMYh($resultado["corr_arch_ret_conf"]);
	...
	$smarty->display('mensajes/eliminar_en_archivo.html');
	die();
?>
