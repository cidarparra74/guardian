...
require_once('../lib/fechas.php');
	...
	$sql= "SELECT m.id_carpeta, m.corr_auto, m.auto_arch, m.arch_corr_prest, m.arch_corr_plazo, 
	m.arch_corr_conf, m.corr_arch_ret, m.id_us_corriente, m.id_us_autoriza, m.id_us_archivo,
	m.obs_1, m.obs_3, m.obs_4, m.obs_5, m.obs_6, u.nombres  
	FROM movimientos_carpetas m, usuarios u 
	WHERE id_movimiento_carpeta='$id_movimiento' AND m.flujo='0' AND m.id_estado='6' AND m.id_us_corriente=u.id_usuario ";
	//echo "$sql";
	$result= consulta($sql);
	...
	$fecha_arch_corr_conf= fechaDMYh($resultado["arch_corr_conf"]);
	$fecha_corr_arch_ret= fechaDMYh($resultado["corr_arch_ret"]);
	...
	//firma 
	$aux= $resultado["id_us_archivo"];
	$sql_a= "SELECT * FROM usuarios WHERE id_usuario='$aux' ";
	$result_a= consulta($sql_a);
	$row_a= $result_a->fetchRow(DB_FETCHMODE_ASSOC);
	$nombre_us_actual= $row_a["nombres"];   // verificar que sea este !!
	$smarty->assign('nombre_us_actual',$nombre_us_actual);
	
	//id de la carpeta
	$id= $id_carpeta;
	//recuperando los datos de la carpeta
	$sql= "SELECT c.carpeta, t.tipo_bien, o.nombre, p.mis, p.nombres 
	FROM carpetas c, tipos_bien t, oficinas o, propietarios p 
	WHERE c.id_tipo_carpeta=t.id_tipo_bien AND c.id_oficina=o.id_oficina AND c.id_carpeta='$id' AND c.id_propietario=p.id_propietario ";
	$result= consulta($sql);
	$resultado= $result->fetchRow(DB_FETCHMODE_ASSOC);
	...
	$smarty->display('mensajes/confirmar_retorno.html');
	die();
?>
