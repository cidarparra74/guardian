<?php
	...
	m.arch_corr_conf, m.corr_arch_ret, m.corr_dev, m.id_us_corriente, m.id_us_autoriza, m.obs_1, 
	m.obs_3, m.obs_4, m.obs_5, m.obs_6, m.obs_8, u.nombres  
	FROM movimientos_carpetas m, usuarios u 
	WHERE id_movimiento_carpeta='$id_movimiento' AND m.id_estado='8' ";
	/*$sql= "SELECT m.id_movimiento_carpeta, m.id_carpeta, m.corr_auto, m.auto_arch, m.auto_arch_plazo, 
		...
		WHERE c.id_oficina=o.id_oficina AND  m.id_carpeta='$id_movimiento' 
		AND c.id_carpeta=m.id_carpeta  AND m.id_estado='8' 
		AND m.id_us_corriente=u.id_usuario ORDER BY u.nombres, m.corr_dev ";*/
	//echo "$sql";
	$result= consulta($sql);
	...
	$fecha_corr_dev= $resultado["corr_dev"];
	...
	$observacion_corr_dev= $resultado["obs_8"];
	...
	$sql= "SELECT c.carpeta, t.tipo_bien as tipo, o.nombre, p.mis, p.nombres 
	...
	$smarty->assign('fecha_corr_dev',$fecha_corr_dev);
	...
	$smarty->assign('observacion_corr_dev',$observacion_corr_dev);

	$smarty->display('mensajes/eliminar_devuelta_cliente.html');
	die();
?>
