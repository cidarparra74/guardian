...
require_once('../lib/fechas.php');
	...
	$sql= "SELECT m.id_carpeta, m.corr_auto, m.auto_arch, m.arch_corr_prest, m.arch_corr_plazo, m.arch_corr_conf, m.corr_arch_ret, m.id_us_archivo, m.id_us_autoriza, m.obs_1, m.obs_3, m.obs_4, m.obs_5, u.nombres FROM movimientos_carpetas m, usuarios u WHERE id_movimiento_carpeta='$id_movimiento' AND m.flujo='0' AND m.id_estado='5' AND m.id_us_archivo=u.id_usuario ";
	$result= consulta($sql);
	...
	$smarty->assign('m_codigo',$m_codigo);
	$smarty->assign('m_operacion',$m_operacion);
	$smarty->assign('m_doc_identidad',$m_doc_identidad);
	$smarty->assign('m_nombre',$m_nombre);
	$smarty->assign('m_tipo_deudor',$m_tipo_deudor);
	$smarty->assign('m_tipo_garantia',$m_tipo_garantia);
	$smarty->assign('m_valor',$m_valor);
	$smarty->assign('m_moneda',$m_moneda);
	$smarty->assign('m_estado',$m_estado);
				
	$smarty->display('mensajes/devolver_propietario.html');
	die();
?>
