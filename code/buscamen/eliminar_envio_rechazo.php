<?php
	...
	$sql= "SELECT m.id_carpeta, m.corr_auto, m.auto_arch, m.auto_arch_plazo, m.id_us_corriente, m.id_us_archivo, m.obs_1, m.obs_2, u.nombres  FROM movimientos_carpetas m, usuarios u WHERE id_movimiento_carpeta='$id_movimiento' AND m.id_us_corriente=u.id_usuario ";
	$result= consulta($sql);
	...
	while($row= $result->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$nombres_inicio[$i]= $row["apellidos"]." ".$row["nombres"];
	...
	$smarty->display('mensajes/eliminar_envio_rechazo.html');
	die();
?>
