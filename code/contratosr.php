...
//cargando para el overlib
	...
	//obtenemos la localizacion de este usuario, ahora de guardian
	if(!isset($_SESSION["cumpas"])){
		$id_almacen= $_SESSION['id_almacen'];
		require('../lib/conexionMNU.php');
		$sql="SELECT login, nombres FROM usuarios us
				INNER JOIN oficinas ag ON ag.id_oficina = us.id_oficina	
				WHERE ag.id_almacen = $id_almacen AND us.activo='S'
				ORDER BY us.nombres";
		$query= consulta($sql);
		$cumpas="'x'";
		$usuarios = array();
		...
		while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
			$cumpas .= ",'".$row['login']."'";
			$usuarios[] = array('glogin'=>$row['login'], 'nombre'=>$row['nombres']);
		}
		//echo $cumpas;
		$_SESSION["cumpas"] = $cumpas;
		$_SESSION["usuarios"] = $usuarios;
		require('../lib/conexionSEC.php');
	}else{
		$cumpas = $_SESSION["cumpas"];
	...
	if(isset($_REQUEST['buscar_boton'])){
		...
		$filtro_usr= $_SESSION["filtro_usr"];
		$filtro_fecha= $_SESSION["filtro_fecha"];
		$filtro_fech2= $_SESSION["filtro_fech2"];
	}	
	$del_filtro='';	
	
	if($filtro_firma != "-"){ //para caso de usuarios definidos en SEC
		//firma
		if($filtro_firma != "*"){
			$del_filtro= "AND cf.firmado = '$filtro_firma' ";
		}
	}
	if($filtro_usr != "-"){ //para caso de usuarios definidos en SEC
		//usr
		if($filtro_usr != "*"){
			$del_filtro .= "AND us.login = '$filtro_usr' ";
		}else{
			$del_filtro .= "AND us.login in ($cumpas) ";
		}
	}
	
	//texto
	if($filtro_texto != ''){
		$del_filtro .= "AND cf.contenido_sec LIKE '%$filtro_texto%' ";
	}
	...
	if($filtro_fecha != '' && $filtro_fech2 != ''){
		...
		$_SESSION["filtro_usr"]  = $filtro_usr;
...
if($quien == "2"){
	...
	//para q salga la lista de usuarios
	$smarty->assign('lusr','S');
}elseif($quien == "5"){
	...
	AND cf.firmado <> '0' ".
	$del_filtro." ORDER BY cf.fechahora";
//para q salga la lista de usuarios //us.login in ($cumpas) AND
	$smarty->assign('lusr','S');	
}else{
	...
	//para q NO salga la lista de usuarios
	$smarty->assign('lusr','N');
}
