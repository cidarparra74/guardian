...
//cargando para el overlib
	...
	//href
	$carpeta_entrar="_main.php?action=contratosrep2.php";
	$smarty->assign('carpeta_entrar',$carpeta_entrar);
	//form
	$carpeta_acc= "contratosrep2";
	...
	//almacen
	if($filtro_alma != '*'){
		$del_filtro= "AND al.id_almacen = '$filtro_alma' ";
	}
...
/*	require_once('../lib/conexionSEC.php');
	...
	//usuarios
	$sql= "SELECT localizacion, appaterno, apmaterno, nombres, login FROM usuario ORDER BY localizacion, appaterno, apmaterno ";  
	$query= consulta($sql);
	$usuarios = array();
	while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
		$usuarios[]= array('id' => $row["id_almacen"],
							'titulo' => $row["nombre"]);
	}
...
if(!isset($_REQUEST['listar_boton'])){
		...
		$smarty->display('contratosrep2.html');
		die();
}
