...
/**********************valores para la ventana*************************/
	...
	if(isset($_REQUEST["prestar_carpeta"])){
		include("./solicitud_carpeta/prestar.php");
	}
	
	//prestando carpeta
	if(isset($_REQUEST['prestar_boton_x'])){
		include("./solicitud_carpeta/prestando.php");
	}
...
o.nombre AS o_nombre, o.id_almacen, o.id_oficina, 
...
while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$sql_a= "SELECT m.id_carpeta, m.id_estado, m.flujo, m.id_us_corriente, u.nombres 
			...
			WHERE ((m.id_estado='8' AND m.flujo='1') OR m.flujo=0) 
			AND m.id_carpeta='$aux' AND m.id_us_corriente=u.id_usuario ";
	...
	if($enable_catofi=='S' and $row["id_oficina"]!=$_SESSION["id_oficina"]){
		$los_estados = 0;
		$puede_prestar="no";
		$puede_modificar= "no";
	}
	...
	$smarty->display('solicitud_carpeta/lista_carpetas.html');
	die();
