	...
	if(isset($_REQUEST["adicionar"])){
		...
		if($ci_cliente!=''){
			...
			//		$smarty->assign('mone',$mone);
				...
				// caso banco SOL
				//usamos ci_cliente como nro de cuenta, lo pasamos a otra variable  para no confundir
				$nrocaso = $ci_cliente;  //es el nro de cuenta en realidad
				$cuenta = $ci_cliente; // se tomara este en el WS
				$documento = '';
				$nombres = '';
				$emision = ''; // el WS devuelve la emision !
				$personanatural = 'F';
				/////require_once('ws_nrocaso_bsol.php');
				require_once('ws_cuenta.php');
				//si cuenta es encontrada entonces $documento <> ''
				if($documento!=''){
					// existe, buscamos el propietario por CI
					$ci_cliente = $documento;
					$sql = "SELECT ci, id_propietario, nombres FROM propietarios WHERE ci = '$documento'";
					$query = consulta($sql);
					$row= $query->fetchRow(DB_FETCHMODE_ASSOC);
					if($documento != $row["ci"]){
					// no existe en guardian, buscamos con el WS mediante CI
					//esto ya se hace en el ws cuentaCartera (ws_cuenta.php)
						...
						if(trim($nombres) != ''){
							...
							*/
								...
								$sql= "INSERT INTO propietarios (nombres, ci, direccion,
									...
									VALUES('$nombres', '$ci_cliente', '$direccion',
									'$telefonos', $fecha_actual, '$ecivil', '$ci_cliente', '', '$ci_cliente','$nombres','2') ";
							...
							//no existe
							$smarty->assign('vertodo','N');
							$smarty->assign('alerta','No se obtuvo el nombre del cliente. (WS)');
						}
...
$sql = "SELECT ile.id_informe_legal, p.nombres cliente, tb.tipo_bien, tb.con_inf_legal, ile.fecha_recepcion, 
...
$sql = "SELECT TOP 20 ile.id_informe_legal, p.nombres cliente, tb.tipo_bien, tb.con_inf_legal, ile.fecha_recepcion, 
	ile.estado, ile.nrocaso, ofi.nombre as oficina, us.nombres as usuario
	FROM informes_legales ile 
	INNER JOIN usuarios us   ON us.id_usuario  =ile.id_us_comun 
	INNER JOIN oficinas ofi ON ofi.id_oficina = ile.id_oficina 
...
$sql = "SELECT il.id_informe_legal, il.cliente, tb.tipo_bien, us.nombres, 
	il.fecha, il.fecha_solicitud, il.estado, tb.con_inf_legal
