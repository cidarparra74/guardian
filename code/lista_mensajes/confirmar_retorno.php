<?php
	...
	$sql= "SELECT m.id_carpeta, m.corr_auto, m.auto_arch, m.arch_corr_prest, m.arch_corr_plazo, m.arch_corr_conf, m.corr_arch_ret, m.id_us_corriente, m.id_us_autoriza, m.obs_1, m.obs_3, m.obs_4, m.obs_5, m.obs_6, u.nombres  FROM movimientos_carpetas m, usuarios u WHERE id_movimiento_carpeta='$id_movimiento' AND m.flujo='0' AND m.id_estado='6' AND m.id_us_corriente=u.id_usuario ";
	//echo "$sql";
	$result= consulta($sql);
	...
	$smarty->display('lista_mensajes/confirmar_retorno.html');
	die();
?>
