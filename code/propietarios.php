...
//cargando para el overlib
	...
	//href
	$carpeta_entrar="_main.php?action=propietarios.php";
	...
	//ver si puede eliminar
	if(isset($_REQUEST['edoc'])){
		$_SESSION["eliminar_docs"]= "S";
	}
	...
	if(isset($_REQUEST['buscar_boton'])){
		...
		if($aux != ""){
			if($band == 0){
				$del_filtro= " p.telefonos LIKE '%$aux%' ";
				$band=1;
			}
			else{
		...
		if(isset($_REQUEST['filtro_direccion'])){
			...
			if($aux != ""){
				if($band == 0){
					$del_filtro= " p.direccion LIKE '%$aux%' ";
					$band=1;
				}else{
...
if($del_filtro != "nada"){
	...
	$sql= "SELECT TOP 18 p.id_propietario, p.nombres, p.mis, p.ci, 
	p.direccion, p.telefonos, COUNT(c.id_carpeta) AS cantidad , (SELECT COUNT(*) FROM informes_legales WHERE id_propietario=p.id_propietario) as tiene_il
	...
	while($row= $query->fetchRow(DB_FETCHMODE_ASSOC)){
		$ids_propietario[] = array( 'ids' => $row["id_propietario"],
									...
									'tiene_il' => $row["tiene_il"]);
	}
}
	$smarty->assign('ids_propietario',$ids_propietario);
	if(isset($solover) and $solover==1)
		$smarty->display('propietariosver.html');
	else
		$smarty->display('propietarios.html');
	die();
